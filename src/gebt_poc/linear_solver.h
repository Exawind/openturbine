...
inline void solve_linear_system(Kokkos::View<double**> system, Kokkos::View<double*> solution) {
    ...
    using solve = KokkosBatched::TeamVectorSolveUTV<member_type, KokkosBatched::Algo::UTV::Unblocked>;
    ...
    auto n = A.extent(0);
    auto scratch_size = 2 * matrix::shmem_size(n, n) + vector::shmem_size(3*n) + scalar_vector::shmem_size(n);
