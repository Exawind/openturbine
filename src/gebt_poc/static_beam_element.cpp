...
Kokkos::View<double*> StaticBeamLinearizationParameters::ResidualVector(
    ...
    auto residual_gen_coords = Kokkos::subview(residual_vector, Kokkos::make_pair(0, 30));
    auto gen_coords_1D = Kokkos::View<double*>("gen_coords_1D", size_dofs);
    auto populate_gen_coords_1D = KOKKOS_LAMBDA(size_t i) {
        gen_coords_1D(i) =
            gen_coords(i / kNumberOfLieAlgebraComponents, i % kNumberOfLieAlgebraComponents);
    };
    Kokkos::parallel_for(1, populate_gen_coords_1D);

    CalculateStaticResidual(
        position_vectors_, gen_coords_1D, stiffness_matrix_, quadrature_, residual_gen_coords
    );

    auto constraints_residual =
        Kokkos::subview(residual_vector, Kokkos::pair(size_dofs, size_dofs + size_constraints));
    ConstraintsResidualVector(gen_coords_1D, position_vectors_, constraints_residual);
