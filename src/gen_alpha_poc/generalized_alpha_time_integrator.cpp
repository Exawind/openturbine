...
void GeneralizedAlphaTimeIntegrator::UpdateGeneralizedCoordinates(
    ...
    for (size_t node = 0; node < gen_coords.extent(0); node++) {
        ...
        auto current_position =
            Vector{gen_coords(node, 0), gen_coords(node, 1), gen_coords(node, 2)};
        auto updated_position =
            Vector{delta_gen_coords(node, 0), delta_gen_coords(node, 1), delta_gen_coords(node, 2)};
        ...
        auto updated_orientation = quaternion_from_rotation_vector(
            ...
            Vector{delta_gen_coords(node, 3), delta_gen_coords(node, 4), delta_gen_coords(node, 5)} *
            h
        );
