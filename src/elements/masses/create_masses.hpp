...
inline Masses CreateMasses(const MassesInput& masses_input, const std::vector<Node>& nodes) {
    ...
    auto host_gravity = Kokkos::create_mirror_view(masses.gravity);
    auto host_state_indices = Kokkos::create_mirror_view(masses.state_indices);
    auto host_x0 = Kokkos::create_mirror_view(masses.node_x0);
    auto host_Mstar = Kokkos::create_mirror_view(masses.qp_Mstar);
