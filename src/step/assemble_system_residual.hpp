...
inline void AssembleSystemResidual(Solver& solver, Elements& elements) {
    ...
    auto beams_vector_policy = Kokkos::TeamPolicy<>(static_cast<int>(elements.beams.num_elems), Kokkos::AUTO());
    auto masses_vector_policy = Kokkos::RangePolicy<>(0, static_cast<int>(elements.masses.num_elems));
    ...
    Kokkos::parallel_for(
        ...
        ContributeBeamsToVector{
            elements.beams.num_nodes_per_element, elements.beams.element_freedom_table, elements.beams.residual_vector_terms,
            solver.R
        }
    );
    Kokkos::fence();
    Kokkos::parallel_for(
        ...
        ContributeMassesToVector{
            elements.masses.element_freedom_table, elements.masses.residual_vector_terms,
            solver.R
        }
    );
}
