...
double* heat_conduction_solver(int axis_size, double side_length, int n_max, double k, double ic_x0,
    ...
    std::vector<double> axis_points{heat_solve::linspace(0.0, side_length, axis_size)};
    ...
    for (int n = 0; n < n_max; n++) {
        ...
        deltaU = heat_solve::kokkos_laplacian(axis_size, U_im1, dx);
        U = heat_solve::kokkos_1d_heat_conduction(axis_size, U_im1, dt, k, deltaU);
...
std::vector<double> linspace(T start_in, T end_in, int num_in) {
    ...
    if (num < 0) {
        throw std::invalid_argument("linspace: num_in must be a positive integer");
    }
    if (num == 0) {
        return linspaced;
    }
