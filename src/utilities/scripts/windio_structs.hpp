...
// The field assembly includes nine entries that aim at describing the overall configuration of the
// wind turbine
struct Assembly {
    std::string turbine_class;     // IEC wind class. The entry should be :code:`I`, :code:`II`,
                                   // :code:`III`, or :code:`IV`.
    std::string turbulence_class;  // IEC turbulence class of the wind turbine. The options are
                                   // :code:`A`, :code:`B`, :code:`C`, and :code:`D`.
    std::string
        drivetrain;  // Drivetrain configuration. This is intended to inform an automated interpreter
                     // of the yaml about the data specified in the field drivetrain
    std::string rotor_orientation;  // Switch between :code:`upwind` and :code:`downwind` rotor
                                    // configurations.
    int number_of_blades;           // Integer setting the number of blades of the rotor
    double rotor_diameter;  // Rotor diameter, defined as the sum of hub diameter and two times the
                            // blade length along its z axis, see blade
...
// The array :code:`labels` specifies the names of the airfoils to be placed along the blade. The
// positions are specified in the field :code:`grid`. The two arrays must share the same length and
// to keep an airfoil constant along blade span, this must be defined twice. The :code:`labels` must
// match the :code:`names` of the airfoils listed in the top level :code:`airfoils`. In between
// airfoils, the recommended interpolation scheme for both coordinates and polars is the Piecewise
// Cubic Hermite Interpolating Polynomial (PCHIP), which is implemented in `Matlab
// <https://www.mathworks.com/help/matlab/ref/pchip.html>`_ and in the Python library `SciPy
// <https://docs.scipy.org/doc/scipy/reference/generated/scipy.interpolate.PchipInterpolator.html>`_.
// Optionally, the field :code:`rthick.values` can also be defined, see below.
...
// The array :code:`values` specifies the aerodynamic twist along blade span. Twist is generally
// positive toward blade root, and may become negative toward blade tip.
struct Twist {
    std::vector<double> grid;
    std::vector<double> values;
...
// This parameter controls the airfoil position relative to the reference axis, by specifying the
// distance in meters along the chordline from the reference axis to the leading edge. 0 means that
// the airfoil is pinned at the leading edge, a positive offset means that the leading edge is
// upstream of the reference axis in local chordline coordinates, and a negative offset that the
// leading edge aft of the reference axis.
...
// This parameter controls the airfoil position relative to the reference axis, by specifying the
// chordline normal distance in meters from the reference axis. 0 means that the reference axis lies
// on the airfoil chordline, a positive offset means that the chordline is shifted in the direction
// of the suction side relative to the reference axis, and a negative offset that the section is
// shifted in the direction of the pressure side of the airfoil.
...
// This array has recently been added to windIO to overcome the uncertainty in the interpolated
// distribution of relative thickness along blade span. It should match the field
// :code:`airfoil_position`
struct Rthick {
    std::vector<double> grid;
    std::vector<double> values;
...
// For the blade, the reference system is located at blade root, with z aligned with the pitch axis,
// x pointing towards the suction sides of the airfoils (standard prebend will be negative) and y
// pointing to the trailing edge (standard sweep will be positive). The blade coordinate system is
// the one of `BeamDyn
// <https://openfast.readthedocs.io/en/master/source/user/beamdyn/input_files.html#beamdyn-primary-input-file>`_
// of OpenFAST and it is shown in the figure below. The consequences of this reference system is that
// standard wind turbine blades have positive twist inboard and close to zero or even slightly
// negative twist outboard, zero or negative x values for standard prebent blades, and positive y
// values for backward swept blades. The blade main direction is expressed along z, and total blade
// length must be computed integrating the fields x, y, and z three-dimensionally. For tower and
// moniopile, :code:`x` is parallel to the ground pointing downwind, :code:`y` is parallel to the
// ground and to the rotor plane, and :code:`z` is perpendicular to the ground pointing upwards.
// Standard monopiles are only defined along :code:`z`.
...
// :code:`outer_shape_bem` consists of a dictionary containing the data for blade BEM-based
// aerodynamics.
struct OuterShapeBem {
    AirfoilPosition
        airfoil_position;  // The array :code:`labels` specifies the names of the airfoils to be
                           // placed along the blade. The positions are specified in the field
                           // :code:`grid`. The two arrays must share the same length and to keep an
                           // airfoil constant along blade span, this must be defined twice. The
                           // :code:`labels` must match the :code:`names` of the airfoils listed in
                           // the top level :code:`airfoils`. In between airfoils, the recommended
                           // interpolation scheme for both coordinates and polars is the Piecewise
                           // Cubic Hermite Interpolating Polynomial (PCHIP), which is implemented in
                           // `Matlab <https://www.mathworks.com/help/matlab/ref/pchip.html>`_ and in
                           // the Python library `SciPy
                           // <https://docs.scipy.org/doc/scipy/reference/generated/scipy.interpolate.PchipInterpolator.html>`_.
                           // Optionally, the field :code:`rthick.values` can also be defined, see
                           // below.
    Chord chord;  // The array :code:`values` specifies the chord along blade span.
    Twist
        twist;  // The array :code:`values` specifies the aerodynamic twist along blade span. Twist
                // is generally positive toward blade root, and may become negative toward blade tip.
    SectionOffsetX
        section_offset_x;  // This parameter controls the airfoil position relative to the reference
                           // axis, by specifying the distance in meters along the chordline from the
                           // reference axis to the leading edge. 0 means that the airfoil is pinned
                           // at the leading edge, a positive offset means that the leading edge is
                           // upstream of the reference axis in local chordline coordinates, and a
                           // negative offset that the leading edge aft of the reference axis.
    SectionOffsetY
        section_offset_y;  // This parameter controls the airfoil position relative to the reference
                           // axis, by specifying the chordline normal distance in meters from the
                           // reference axis. 0 means that the reference axis lies on the airfoil
                           // chordline, a positive offset means that the chordline is shifted in the
                           // direction of the suction side relative to the reference axis, and a
                           // negative offset that the section is shifted in the direction of the
                           // pressure side of the airfoil.
    Rthick rthick;  // This array has recently been added to windIO to overcome the uncertainty in
                    // the interpolated distribution of relative thickness along blade span. It
                    // should match the field :code:`airfoil_position`
...
struct StiffnessMatrix {
    std::vector<double> grid;
    std::vector<double> k11;  // Distribution of the K11 element of the stiffness matrix along blade
                              // span. K11 corresponds to the shear stiffness along the x axis (in a
                              // blade, x points to the trailing edge)
    std::vector<double> k22;  // Distribution of the K22 element of the stiffness matrix along blade
                              // span. K22 corresponds to the shear stiffness along the y axis (in a
                              // blade, y points to the suction side)
    std::vector<double> k33;  // Distribution of the K33 element of the stiffness matrix along blade
                              // span. K33 corresponds to the axial stiffness along the z axis (in a
                              // blade, z runs along the span and points to the tip)
    std::vector<double>
        k44;  // Distribution of the K44 element of the stiffness matrix along blade span. K44
              // corresponds to the bending stiffness around the x axis (in a blade, x points to the
              // trailing edge and K44 corresponds to the flapwise stiffness)
    std::vector<double>
        k55;  // Distribution of the K55 element of the stiffness matrix along blade span. K55
              // corresponds to the bending stiffness around the y axis (in a blade, y points to the
              // suction side and K55 corresponds to the edgewise stiffness)
    std::vector<double> k66;  // Distribution of K66 element of the stiffness matrix along blade
                              // span. K66 corresponds to the torsional stiffness along the z axis
                              // (in a blade, z runs along the span and points to the tip)
    std::vector<double> k12;  // Distribution of the K12 element of the stiffness matrix along blade
                              // span. K12 is a cross term between shear terms
    std::vector<double> k13;  // Distribution of the K13 element of the stiffness matrix along blade
                              // span. K13 is a cross term shear - axial
    std::vector<double> k14;  // Distribution of the K14 element of the stiffness matrix along blade
                              // span. K14 is a cross term shear - bending
    std::vector<double> k15;  // Distribution of the K15 element of the stiffness matrix along blade
                              // span. K15 is a cross term shear - bending
    std::vector<double> k16;  // Distribution of the K16 element of the stiffness matrix along blade
                              // span. K16 is a cross term shear - torsion
    std::vector<double> k23;  // Distribution of the K23 element of the stiffness matrix along blade
                              // span. K23 is a cross term shear - axial
    std::vector<double> k24;  // Distribution of the K24 element of the stiffness matrix along blade
                              // span. K24 is a cross term shear - bending
    std::vector<double> k25;  // Distribution of the K25 element of the stiffness matrix along blade
                              // span. K25 is a cross term shear - bending
    std::vector<double> k26;  // Distribution of the K26 element of the stiffness matrix along blade
                              // span. K26 is a cross term shear - torsion
    std::vector<double> k34;  // Distribution of the K34 element of the stiffness matrix along blade
                              // span. K34 is a cross term axial - bending
    std::vector<double> k35;  // Distribution of the K35 element of the stiffness matrix along blade
                              // span. K35 is a cross term axial - bending
    std::vector<double> k36;  // Distribution of the K36 element of the stiffness matrix along blade
                              // span. K36 is a cross term axial - torsion
    std::vector<double> k45;  // Distribution of the K45 element of the stiffness matrix along blade
                              // span. K45 is a cross term flapwise bending - edgewise bending
    std::vector<double> k46;  // Distribution of the K46 element of the stiffness matrix along blade
                              // span. K46 is a cross term flapwise bending - torsion
    std::vector<double> k56;  // Distribution of the K56 element of the stiffness matrix along blade
                              // span. K56 is a cross term edgewise bending - torsion
...
struct InertiaMatrix {
    std::vector<double> grid;
    std::vector<double>
        mass;  // Mass per unit length along the beam, expressed in kilogram per meter
    std::vector<double>
        cm_x;  // Distance between the reference axis and the center of mass along the x axis
    std::vector<double>
        cm_y;  // Distance between the reference axis and the center of mass along the y axis
    ...
    std::vector<double>
        i_plr;  // Polar moment of inertia per unit span (around z axis). Please note that for
                // beam-like structures iplr must be equal to iedge plus iflap.
...
struct StructuralDamping {
    std::vector<double> mu;  // Six damping coefficients to model viscous damping, where the damping
                             // forces are proportional to the strain rate
...
struct PointMass {
    std::vector<double> grid;
    std::vector<double> mass;  // Point masses distributed along the beam, expressed in kilogram.
                               // These can be used to model features such as flanges.
...
// The equivalent elastic properties of a beam are defined in :code:`elastic_properties_mb`. Here,
// 6x6 stiffness and mass matrices are defined. Out of 36 entries of the matrices, given the
// symmetry, the yaml file requires the definition of only 21 values as inputs for the stiffness
// matrix, whereas the inertia matrix is defined in terms of unit mass, coordinates of the center of
...
struct ElasticPropertiesMb {
    ...
    StructuralDamping
        structural_damping;  // Structural damping of the beam. For now, viscous damping is included
...
struct StartNdArc {
    std::vector<double> grid;
    std::vector<double> values;  // Grid along an arc length, expressed non dimensionally where 0 is
                                 // the trailing edge on the suction side and 1 is the trailing edge
                                 // on the pressure side. The arc starts and ends at the mid-point of
                                 // the trailing edge in the case of open trailing edges.
...
// The rotation defines the angle between the chord line and the y axis and it has the opposite sign
// of the twist. For shear webs perpendicular to the chord line in the section(s) where twist is
// zero, it is easiest to simply use the keyword fixed :code:`twist`. Blades with straight shear webs
// will always have the field rotation equal to the twist plus/minus a constant angle and, assuming a
// non-swept blade (zero values in the blade y reference axis), a linear field offset.
struct Rotation {
    std::vector<double> grid;
    std::vector<double> values;
...
// Dimensional offset in respect to the reference axis along the x axis, which is the chord line
// rotated by a user-defined angle. Negative values move the midpoint towards the leading edge,
...
// Dimensional thickness of the laminate, expressed in meters. This value is modeled constant along
// the section. To define ply drops along the 2D surface, the user is therefore required to define
// multiple layers, possibly ply by ply when many ply drops exist.
struct Thickness {
    std::vector<double> grid;
    std::vector<double> values;
...
// In addition or in alternative to the dimensional thickness, the discrete number of plies of a
// composite laminate can be defined by the user. Notably, the ply thickness is a material property
// (not a layer property) and it is defined in the top-level field :code:`materials`.
struct NPlies {
    std::vector<double> grid;
    std::vector<double> values;
...
// For composite laminates, the orientation of the fibers in degrees can be specified. Looking from
// blade root, positive angles represent a rotation of the fibers towards the leading edge of the
// blade. Note that the angles are with respect to the cross section local reference system, not the
// reference system at blade root.
...
struct Layers {
    ...
    std::string material;  // String that identifies the material of the layer. The material and its
                           // properties must be defined in the top-level :code:`materials`.
    std::string web;  // web to which the layer is associated to, only to be defined for web layers
    Thickness thickness;  // Dimensional thickness of the laminate, expressed in meters. This value
                          // is modeled constant along the section. To define ply drops along the 2D
                          // surface, the user is therefore required to define multiple layers,
                          // possibly ply by ply when many ply drops exist.
    NPlies n_plies;       // In addition or in alternative to the dimensional thickness, the discrete
                     // number of plies of a composite laminate can be defined by the user. Notably,
                     // the ply thickness is a material property (not a layer property) and it is
                     // defined in the top-level field :code:`materials`.
    FiberOrientation
        fiber_orientation;  // For composite laminates, the orientation of the fibers in degrees can
                            // be specified. Looking from blade root, positive angles represent a
                            // rotation of the fibers towards the leading edge of the blade. Note
                            // that the angles are with respect to the cross section local reference
                            // system, not the reference system at blade root.
    Width width;  // The field width defines the width in meters along the arc of the layer.
    MidpointNdArc midpoint_nd_arc;
    std::string side;  // The field side is string that can be either :code:`suction` or
                       // :code:`pressure`, defining the side where a layer is defined.
    StartNdArc start_nd_arc;
    EndNdArc end_nd_arc;
    Rotation rotation;
    OffsetYRefAxis offset_y_ref_axis;
...
// The field :code:`internal_structure_2d_fem` contains the data to describe the internal structure
// of standard wind turbine blades. This is a fairly sophisticated process and the ontology proposed
// in this work supports different definitions. On the top level, the field
// :code:`internal_structure_2d_fem` has three sub-components, namely the :code:`reference_axis`,
// which is usually defined equal to the :code:`reference_axis` in the field :code:`outer_shape_bem`,
// the :code:`webs`, where the positions of the shear webs are defined, and the :code:`layers`, which
// describe all internal layers in terms of :code:`name`, :code:`material`, :code:`thickness`, number
// of plies :code:`n_plies`, :code:`fiber_orientation` (for composites), and position in the
// two-dimensional sections. Recently, the fields :code:`joint` and :code:`root` were added to
// support blades that are segmented spanwise and details about the blade root bolting.
struct InternalStructure2DFem {
    Root root;
    ReferenceAxis reference_axis;
    std::vector<Webs>
        webs;  // The field :code:`webs` consists of a list of entries, each representing a shear web
               // defined in terms of :code:`name` and position.
    std::vector<Layers>
        layers;  // The sub-field :code:`layers` defines the material layers of the wind turbine
                 // blade. In most cases, these are layers of composite materials. The most
                 // convenient approach to define the position of spar caps mimics the definition of
                 // the shear webs, adding the width and side that define the width of the layer in
                 // meters and the side where the layer is located, either “pressure” or “suction”.
                 // Layers that wrap the entire cross section, such as paint or outer shell skin, can
                 // be defined with :code:`start_nd_arc` set to 0 and :code:`end_nd_arc` set to 1. To
                 // define reinforcements, the best way is usually to define the width, in meters,
                 // and the midpoint, named :code:`midpoint_nd_arc` and defined nondimensional
                 // between 0 and 1. Converters should be able to look for the leading edge, marked
                 // as LE. Similar combinations can be constructed with the combination of
                 // :code:`width` and :code:`start_nd_arc` or :code:`end_nd_arc`. Finally, for
                 // composite layers belonging to the shear webs, a tag :code:`web` should contain
                 // the name of the web. The layers are then modeled from leading edge to trailing
                 // edge in the order they were specified.
...
// The component :code:`blade` includes three subcomponents, namely :code:`outer_shape_bem`,
// :code:`elastic_properties_mb`, and :code:`internal_structure_2d_fem`. A fourth and a fifth
// subfields :code:`cfd_geometry` and :code:`3D_fem` will be added to support higher fidelity
// modeling of the rotor. All distributed quantities, such as blade chord or the thickness of a
// structural component, are expressed in terms of pair arrays :code:`grid` and :code:`values`, which
// must have a minimum length of two elements and the same size. :code:`grid` is defined
// nondimensional between 0 (root) and 1 (tip) along the, possibly curved, :code:`reference_axis`.
struct Blade {
    OuterShapeBem outer_shape_bem;  // :code:`outer_shape_bem` consists of a dictionary containing
                                    // the data for blade BEM-based aerodynamics.
    ...
    InternalStructure2DFem
        internal_structure_2d_fem;  // The field :code:`internal_structure_2d_fem` contains the data
                                    // to describe the internal structure of standard wind turbine
                                    // blades. This is a fairly sophisticated process and the
                                    // ontology proposed in this work supports different definitions.
                                    // On the top level, the field :code:`internal_structure_2d_fem`
                                    // has three sub-components, namely the :code:`reference_axis`,
                                    // which is usually defined equal to the :code:`reference_axis`
                                    // in the field :code:`outer_shape_bem`, the :code:`webs`, where
                                    // the positions of the shear webs are defined, and the
                                    // :code:`layers`, which describe all internal layers in terms of
                                    // :code:`name`, :code:`material`, :code:`thickness`, number of
                                    // plies :code:`n_plies`, :code:`fiber_orientation` (for
                                    // composites), and position in the two-dimensional sections.
                                    // Recently, the fields :code:`joint` and :code:`root` were added
                                    // to support blades that are segmented spanwise and details
                                    // about the blade root bolting.
...
struct OuterShapeBem_1 {
    double diameter;  // This is the outer diameter of the hub. It is also the diameter of the circle
                      // centered at the rotor apex and connecting the blade root centers.
    double cone_angle;  // Rotor precone angle, defined positive for both upwind and downwind rotors.
    double
        cd;  // Equivalent drag coefficient to compute the aerodynamic forces generated on the hub.
...
struct ElasticPropertiesMb_1 {
    ...
    std::vector<double>
        location;  // Location of the point mass with respect to the coordinate system
    std::string
        coordinate_system;  // Coordinate system used to define the location of the point mass

    void parse(const YAML::Node& node) {
        mass = node["mass"] ? node["mass"].as<double>() : 0.;
        inertia =
            node["inertia"] ? node["inertia"].as<std::vector<double>>() : std::vector<double>();
        location =
            node["location"] ? node["location"].as<std::vector<double>>() : std::vector<double>();
        coordinate_system =
            node["coordinate_system"] ? node["coordinate_system"].as<std::string>() : "";
    }
};
...
struct Hub {
    ...
    ElasticPropertiesMb_1 elastic_properties_mb;  // Point mass modeling the full hub system, which
                                                  // includes the hub, the spinner, the blade
...
// Geometrical metrics describing the drivetrain. Currently, these are inputs to WISDEM specific
// drivetrain sizing tool, DrivetrainSE
...
struct Gearbox {
    ...
    double length;      // User input override of gearbox length along shaft, only used when using
                        // gearbox_mass_user is > 0
    double radius;  // User input override of gearbox radius, only used when using gearbox_mass_user
                    // is > 0
...
struct Lss {
    ...
    std::vector<double> diameter;               // Diameter of the low speed shaft at beginning
                                                // (generator/gearbox) and end (hub) points
    std::vector<double> wall_thickness;         // Thickness of the low speed shaft at beginning
                                                // (generator/gearbox) and end (hub) points
    ...
    void parse(const YAML::Node& node) {
        length = node["length"] ? node["length"].as<double>() : 0.;
        diameter =
            node["diameter"] ? node["diameter"].as<std::vector<double>>() : std::vector<double>();
        wall_thickness = node["wall_thickness"] ? node["wall_thickness"].as<std::vector<double>>()
                                                : std::vector<double>();
        material = node["material"] ? node["material"].as<std::string>() : "";
        if (node["elastic_properties_mb"]) {
            elastic_properties_mb.parse(node["elastic_properties_mb"]);
...
struct Hss {
    ...
    std::vector<double> diameter;  // Diameter of the high speed shaft at beginning (generator) and
                                   // end (generator) points
    std::vector<double> wall_thickness;         // Thickness of the high speed shaft at beginning
...
struct Nose {
    std::vector<double> diameter;  // Diameter of the nose/turret at beginning (bedplate) and end
                                   // (main bearing) points
    std::vector<double> wall_thickness;  // Thickness of the nose/turret at beginning (bedplate) and
...
struct Bedplate {
    WallThickness wall_thickness;   // Thickness of the hollow elliptical bedplate used in direct
...
// Inputs describing all other drivetrain components, the assembly of brake, hvac, converter,
// transformer, and main bearings
struct OtherComponents {
    double
        brake_mass;  // Override regular regression-based calculation of brake mass with this value
    double hvac_mass_coefficient;  // Regression-based scaling coefficient on machine rating to get
                                   // HVAC system mass
    double converter_mass;    // Override regular regression-based calculation of converter mass with
                              // this value
    double transformer_mass;  // Override regular regression-based calculation of transformer mass
                              // with this value
    ...
    ElasticPropertiesMb_3 elastic_properties_mb;  // Point mass modeling the assembly of brake, hvac,
                                                  // converter, transformer, and main bearings
...
struct Generator {
    ...
    double radius;  // User input override of generator radius, only used when using simple generator
                    // scaling
    double mass;    // User input override of generator mass, only used when using simple generator
                    // mass scaling
    RpmEfficiency rpm_efficiency;  // User input override of generator rpm-efficiency values, with
                                   // rpm as grid input and eff as values input
...
struct Drivetrain {
    OuterShapeBem_2
        outer_shape_bem;  // Geometrical metrics describing the drivetrain. Currently, these are
    ...
    Nose nose;  // Inputs describing the nose/turret at beginning (bedplate) and end (main bearing)
                // points
    Bedplate bedplate;  // Inputs describing the hollow elliptical bedplate used in direct drive
                        // configurations
    OtherComponents
        other_components;  // Inputs describing all other drivetrain components, the assembly of
                           // brake, hvac, converter, transformer, and main bearings
    Generator generator;
...
struct OuterShapeBem_3 {
    ReferenceAxis reference_axis;
    OuterDiameter outer_diameter;  // Outer diameters of the tower defined from base (grid = 0) to
                                   // top (grid = 1).
    Cd cd;
...
struct InternalStructure2DFem_1 {
    double outfitting_factor;  // Multiplier of tower mass to account for the mass of the auxiliary
                               // systems, such as stairs, elevator, paint, or extra structural
                               // elements. This can be used to convert the mass of the steel
                               // cylinders to the total mass of the monopile.
    ReferenceAxis reference_axis;
...
struct OuterShapeBem_4 {
    ReferenceAxis reference_axis;
    OuterDiameter outer_diameter;  // Outer diameters of the monopile defined from base (grid = 0) to
                                   // top (grid = 1).
    Ca ca;  // Added mass coefficient for the monopile defined in terms of grid and values that
            // default to 1.
...
struct InternalStructure2DFem_2 {
    double outfitting_factor;  // Multiplier of monopile mass to account for the mass of the
                               // auxiliary systems, such as paint or extra structural elements. This
                               // can be used to convert the mass of the steel cylinders to the total
                               // mass of the monopile.
    ReferenceAxis reference_axis;
    std::vector<Layers> layers;  // ...
...
struct Members {
    ...
    std::vector<AxialJoints>
        axial_joints;  // Additional joints that are defined along the non-dimensional member axis
                       // can be defined here.  Unlike the joints defined in the global coordinate
                       // system in the :code:`joints` section of the ontology, these joints will
                       // change their absolute (x,y,z) location if the member diameter is altered
                       // the or :code:`joint1` or :code:`joint2` locations are changed during an
                       // optimization.  This is especially useful when designing a truss-like
                       // structure with pontoons attaching to primary members or columns in a
                       // semisubmersible. Like the joints above, these must be given a unique name.
...
// The floating platform ontology uses a *graph*-like representation of the geometry with Joints and
// Members.  Additional rigid body point masses can be defined at the joints as well.
struct FloatingPlatform {
    std::vector<Joints>
        joints;  // Joints are the *nodes* of the graph representation of the floating platform. They
                 // must be assigned a unique name for later reference by the members.
    std::vector<Members> members;
    std::vector<RigidBodies>
        rigid_bodies;  // Additional point masses at joints with user-customized properties
    double transition_piece_mass;  // Total mass of transition piece
    double transition_piece_cost;  // Total cost of transition piece
...
// The mooring system ontology follows closely the input file format for MoorDyn or MAP++. Additional
// information can be found in the `MoorDyn user guide
// <http://www.matt-hall.ca/files/MoorDyn-Users-Guide-2017-08-16.pdf>`_ .
struct Mooring {
...
struct Components {
    Blade blade;  // The component :code:`blade` includes three subcomponents, namely
                  // :code:`outer_shape_bem`, :code:`elastic_properties_mb`, and
                  // :code:`internal_structure_2d_fem`. A fourth and a fifth subfields
                  // :code:`cfd_geometry` and :code:`3D_fem` will be added to support higher fidelity
                  // modeling of the rotor. All distributed quantities, such as blade chord or the
                  // thickness of a structural component, are expressed in terms of pair arrays
                  // :code:`grid` and :code:`values`, which must have a minimum length of two
                  // elements and the same size. :code:`grid` is defined nondimensional between 0
                  // (root) and 1 (tip) along the, possibly curved, :code:`reference_axis`.
    Hub hub;
    ...
    Jacket jacket;
    FloatingPlatform
        floating_platform;  // The floating platform ontology uses a *graph*-like representation of
                            // the geometry with Joints and Members.  Additional rigid body point
                            // masses can be defined at the joints as well.
    Mooring mooring;        // The mooring system ontology follows closely the input file format for
                      // MoorDyn or MAP++.  Additional information can be found in the `MoorDyn user
                      // guide <http://www.matt-hall.ca/files/MoorDyn-Users-Guide-2017-08-16.pdf>`_ .
...
// The airfoil :code:`coordinates` are specified here in the sub-fields :code:`x` and :code:`y`.
// :code:`x` and :code:`y` must have the same length. :code:`x` must be defined between 0, which
// corresponds to the leading edge, and 1, which corresponds to the trailing edge. Airfoil
// coordinates should be defined from trailing edge (:code:`x=1`) towards the suction side (mostly
// positive :code:`y` values), to leading edge (:code:`x=0`, :code:`y=0`), to the pressure side
// (mostly negative :code:`y`), and conclude at the trailing edge pressure side (:code:`x=1`).
// Flatback airfoils can be defined either open (:code:`x=1`, :code:`y!=0`) or closed (:code:`x=1`,
// :code:`y=0`).
struct Coordinates {
    std::vector<double> x;
    std::vector<double> y;
...
// List of one or more sets of polars for the airfoil. The field :code:`polars` must include the
// sub-fields :code:`configuration`, :code:`re`, :code:`cl`, :code:`cd`, and :code:`cm`. Parameters
// characterizing the unsteady aerodynamics are optional.
struct Polars {
    ...
    double t_f0;  // Initial value of the time constant associated with Df in the expression of Df
                  // and f. [default = 3]
    double t_v0;  // Initial value of the time constant associated with the vortex lift decay
                  // process; it is used in the expression of Cvn. It depends on Re,M, and airfoil
                  // class. [default = 6]
    double t_p;   // Boundary-layer,leading edge pressure gradient time constant in the expression of
                  // Dp. It should be tuned based on airfoil experimental data. [default = 1.7]
    double t_vl;  // Initial value of the time constant associated with the vortex advection process;
                  // it represents the non-dimensional time in semi-chords, needed for a vortex to
                  // travel from LE to trailing edge (TE); it is used in the expression of Cvn. It
                  // depends on Re, M (weakly), and airfoil. [valid range = 6 - 13, default = 11]
    double b1;    // Constant in the expression of phi_alpha^c and phi_q^c.  This value is relatively
                  // insensitive for thin airfoils, but may be different for turbine airfoils. [from
                  // experimental results, defaults to 0.14]
    double b2;    // Constant in the expression of phi_alpha^c and phi_q^c.  This value is relatively
                  // insensitive for thin airfoils, but may be different for turbine airfoils. [from
                  // experimental results, defaults to 0.53]
    double b5;    // Constant in the expression of K'''_q,cm_q^nc, and k_m,q.  [from  experimental
                  // results, defaults to 5]
    double a1;    // Constant in the expression of phi_alpha^c and phi_q^c.  This value is relatively
                  // insensitive for thin airfoils, but may be different for turbine airfoils. [from
                  // experimental results, defaults to 0.3]
    double a2;    // Constant in the expression of phi_alpha^c and phi_q^c.  This value is relatively
                  // insensitive for thin airfoils, but may be different for turbine airfoils. [from
                  // experimental results, defaults to 0.7]
    double a5;    // Constant in the expression of K'''_q,cm_q^nc, and k_m,q. [from experimental
                  // results, defaults to 1]
    double s1;  // Constant in the f curve best-fit for alpha0<=AOA<=alpha1; by definition it depends
                // on the airfoil.
    double s2;  // Constant in the f curve best-fit for AOA> alpha1; by definition it depends on the
                // airfoil.
    double s3;  // Constant in the f curve best-fit for alpha2<=AOA< alpha0; by definition it depends
                // on the airfoil.
    double s4;  // Constant in the f curve best-fit for AOA< alpha2; by definition it depends on the
                // airfoil.
    double cn1;    // Critical value of C0n at leading edge separation. It should be extracted from
                   // airfoil data at a given Mach and Reynolds number. It can be calculated from the
                   // static value of Cn at either the break in the pitching moment or the loss of
                   // chord force at the onset of stall. It is close to the condition of maximum lift
                   // of the airfoil at low Mach numbers.
    ...
    double cm0;    // 2D pitching moment coefficient about 1/4-chord location, at 0-lift, positive if
                   // nose up.
    ...
    double uacutout;    // Angle of attack above which unsteady aerodynamics are disabled (deg).
                        // [Specifying the string "Default" sets UACutout to 45 degrees]
    double filtcutoff;  // Cut-off frequency (-3 dB corner frequency) for low-pass filtering the AoA
                        // input to UA, as well as the 1st and 2nd derivatives (Hz) [default = 20]
...
struct Airfoils {
    ...
    Coordinates
        coordinates;  // The airfoil :code:`coordinates` are specified here in the sub-fields
                      // :code:`x` and :code:`y`. :code:`x` and :code:`y` must have the same length.
                      // :code:`x` must be defined between 0, which corresponds to the leading edge,
                      // and 1, which corresponds to the trailing edge. Airfoil coordinates should be
                      // defined from trailing edge (:code:`x=1`) towards the suction side (mostly
                      // positive :code:`y` values), to leading edge (:code:`x=0`, :code:`y=0`), to
                      // the pressure side (mostly negative :code:`y`), and conclude at the trailing
                      // edge pressure side (:code:`x=1`). Flatback airfoils can be defined either
                      // open (:code:`x=1`, :code:`y!=0`) or closed (:code:`x=1`, :code:`y=0`).
    double relative_thickness;  // Float defined between 0 (plate) and 1 (cylinder) to specify the
                                // relative thickness of the airfoil. This generates a small
                                // redundancy (airfoil thickness could be determined from the field
                                // coordinates), but it simplifies the converters.
    double aerodynamic_center;  // Float defined between 0 (leading edge) and 1 (trailing edge) to
                                // specify the chordwise coordinate of the aerodynamic center used to
                                // define the polars.
...
struct Materials {
    ...
    std::string description;  // Optional string to describe the origin of the material, for example
                              // referencing a report or a paper
    std::string source;       // Optional field describing where the data come from
    int orth;  // Flag specifying whether a material is isotropic (0) or orthotropic (1). This
               // determines whether some of the fields below are specified as a float or an array of
               // floats.
    ...
    std::variant<double, std::vector<double>>
        alpha;  // Thermal coefficient of expansion. For orthotropic materials, it consists of an
                // array with alpha11, alpha22, and alpha33.
    std::variant<double, std::vector<double>>
    ...
    std::variant<double, std::vector<double>>
        xc;  // Ultimate compressive strength. For orthotropic materials, it consists of an array
             // with Xc11, Xc22, and Xc33. Values are defined positive.
    std::variant<double, std::vector<double>>
    ...
    std::variant<double, std::vector<double>>
        s;  // Ultimate shear strength. For orthotropic materials, it consists of an array with the
            // strength in directions 12, 13 and 23. Values are defined positive.
    double ply_t;      // Ply thickness of a composite material. The unit of measure is m. The actual
                       // laminate thickness is defined in the fields :code:`components`.
    double unit_cost;  // Unit cost of the material. For composites, this is the unit cost of the dry
                       // fabric.
    double fvf;  // Fiber volume fraction of a composite material. The minimum values is 0 (only
                 // matrix), the maximum value is 1 (only fibers).
    double fwf;  // Fiber weight fraction of a composite material. The minimum values is 0 (only
                 // matrix), the maximum value is 1 (only fibers).
    double fiber_density;  // Density of the fibers of a composite material. Standard glass fiber has
                           // a fiber density of approximately 2600 kg/m3, while standard carbon
                           // fiber has a fiber density of approximately 1800 kg/m3.
    double area_density_dry;  // Aerial density of a fabric of a composite material.
    int manufacturing_id;     // Flag to define the manufacturing process behind the laminate, for
                              // example 0 - coating, 1 - sandwich filler , 2 - shell skin, 3 - shear
                              // webs, 4 - spar caps, 5 - TE reinf.
    ...
    std::variant<double, std::vector<double>>
        a;  // Fatigue S/N curve fitting parameter S=A*N^(-1/m).
...
// The field :code:`environment` includes the data characterizing air and water where the wind
// turbine operates.
struct Environment {
...
struct Integrated {
    ...
    double
        electrical_power;  // Eelectrical power of the rotor measured at the output of the generator
...
// Blade deflection with respect to the undeflected configuration along the x axis for the pitching
...
// Blade deflection with respect to the undeflected configuration along the y axis for the pitching
// blade root coordinate system
...
// Blade deflection with respect to the undeflected configuration along the z axis for the pitching
// blade root coordinate system
...
// Blade rotation with respect to the undeflected configuration along the z axis for the pitching
// blade root coordinate system
...
// Distribution along blade span of the reaction force along the x axis (edgewise shear). The force
// follows the pitching blade root coordinate system
...
// Distribution along blade span of the reaction force along the y axis (flapwise shear). The force
// follows the pitching blade root coordinate system
...
// Distribution along blade span of the reaction force along the z axis (axial). The force follows
// the pitching blade root coordinate system
...
// Distribution along blade span of the reaction moment along the x axis (flapwise moment). The
// moment follows the pitching blade root coordinate system
...
// Distribution along blade span of the reaction moment along the y axis (edgewise moment). The
// moment follows the pitching blade root coordinate system
...
// Distribution along blade span of the reaction moment along the z axis (torsional moment). The
// moment follows the pitching blade root coordinate system
...
struct Distributed {
    ...
    AeroForceTangential
        aero_force_tangential;  // Aerodynamic loading along the tangential rotor direction
    BladeTranslationDeflectionXPitching
        blade_translation_deflection_x_pitching;  // Blade deflection with respect to the undeflected
                                                  // configuration along the x axis for the pitching
                                                  // blade root coordinate system
    BladeTranslationDeflectionYPitching
        blade_translation_deflection_y_pitching;  // Blade deflection with respect to the undeflected
                                                  // configuration along the y axis for the pitching
                                                  // blade root coordinate system
    BladeTranslationDeflectionZPitching
        blade_translation_deflection_z_pitching;  // Blade deflection with respect to the undeflected
                                                  // configuration along the z axis for the pitching
                                                  // blade root coordinate system
    BladeRotationDeflectionZPitching
        blade_rotation_deflection_z_pitching;  // Blade rotation with respect to the undeflected
                                               // configuration along the z axis for the pitching
                                               // blade root coordinate system
    BladeFxPitching blade_fx_pitching;  // Distribution along blade span of the reaction force along
                                        // the x axis (edgewise shear). The force follows the
                                        // pitching blade root coordinate system
    BladeFyPitching blade_fy_pitching;  // Distribution along blade span of the reaction force along
                                        // the y axis (flapwise shear). The force follows the
                                        // pitching blade root coordinate system
    BladeFzPitching
        blade_fz_pitching;  // Distribution along blade span of the reaction force along the z axis
                            // (axial). The force follows the pitching blade root coordinate system
    BladeMxPitching blade_mx_pitching;  // Distribution along blade span of the reaction moment along
                                        // the x axis (flapwise moment). The moment follows the
                                        // pitching blade root coordinate system
    BladeMyPitching blade_my_pitching;  // Distribution along blade span of the reaction moment along
                                        // the y axis (edgewise moment). The moment follows the
                                        // pitching blade root coordinate system
    BladeMzPitching blade_mz_pitching;  // Distribution along blade span of the reaction moment along
                                        // the z axis (torsional moment). The moment follows the
                                        // pitching blade root coordinate system
    ...
    void parse(const YAML::Node& node) {
        ...
        if (node["blade_translation_deflection_x_pitching"]) {
            blade_translation_deflection_x_pitching.parse(
                node["blade_translation_deflection_x_pitching"]
            );
        }
        if (node["blade_translation_deflection_y_pitching"]) {
            blade_translation_deflection_y_pitching.parse(
                node["blade_translation_deflection_y_pitching"]
            );
        }
        if (node["blade_translation_deflection_z_pitching"]) {
            blade_translation_deflection_z_pitching.parse(
                node["blade_translation_deflection_z_pitching"]
            );
        }
...
struct Cases {
    ...
    std::vector<double>
        damping_ratio;  // Critical damping ratios of the modes characterizing the system
    ...
    void parse(const YAML::Node& node) {
        ...
        frequency_undamped = node["frequency_undamped"]
                                 ? node["frequency_undamped"].as<std::vector<double>>()
                                 : std::vector<double>();
        damping_ratio = node["damping_ratio"] ? node["damping_ratio"].as<std::vector<double>>()
                                              : std::vector<double>();
    }
};
...
struct Outputs {
    ...
    std::vector<Cases>
        cases;  // Data points where outputs are computed at varying wind speed, rotor speed, etc.
...
struct Turbine {
    std::string
        comments;  // Text field to describe the wind turbine, the changes to previous versions, etc,
    std::string name;            // Unique identifier of the wind turbine model
    std::string windio_version;  // Version of windIO used
    Assembly assembly;      // The field assembly includes nine entries that aim at describing the
                            // overall configuration of the wind turbine
    Components components;  // Nested dictionary structure of components describing the wind turbine
                            // assembly
    std::vector<Airfoils>
        airfoils;  // Database of airfoils. windIO describes the airfoils in terms of coordinates,
                   // polars, and unsteady aerodynamic coefficients. The yaml entry airfoils consists
                   // of a list of elements. For each set of coordinates, multiple sets of polars and
                   // parameters can co-exist. Note that the airfoils listed in this database are not
                   // all necessarily used in the blade. Only the ones called in
                   // :code:`airfoil_position' within :code:`outer_shape_bem` of the
                   // :code:`component` :code:`blade` will actually be loaded to model the blade.
    std::vector<Materials>
        materials;  // Database of the materials. The schema enforces that the fields :code:`name`,
                    // :code:`orth`, :code:`rho`, :code:`E`, and :code:`nu` are specified. For
                    // composites, direction 1 is aligned with the main fiber direction, direction 2
                    // is in the plane transverse to the fibers, and direction 3 is perspendicular to
                    // the laminate plane. Note that fiber angles are specified in
                    // :code:`internal_structure_2d_fem` of the :code:`component` :code:`blade`.
    Control control;          // Dictionary reporting the data describing the wind turbine controller
    Environment environment;  // The field :code:`environment` includes the data characterizing air
                              // and water where the wind turbine operates.
