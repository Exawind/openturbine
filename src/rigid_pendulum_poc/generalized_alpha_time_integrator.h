...
public:
    ...
    /*! @brief  Performs the SolveTimeStep() algorithm of the Lie group based
     * generalized-alpha method as described in the paper by Brüls and Cardona
     * (2010) https://doi.org/10.1115/1.4001370
     * @param   state Current state of the system at the beginning of the time
     * step
...
private:
    ...
    bool is_converged_;  //< Flag to indicate if the latest non-linear update
                         // has converged

    TimeStepper time_stepper_;  //< Time stepper object to perform the time integration

    /// Computes the updated generalized coordinates based on the non-linear
    /// update
    ...
    /// Returns the flag to indicate if the latest non-linear update has
    /// converged
