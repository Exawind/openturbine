    ...
    double residual_tolerance)
{
    ...
    for (int n = 0; n < n_max; n++) {
        ...
        deltaU =
            openturbine::heat_solve::kokkos_laplacian(axis_size, U_im1, dx);
        U = openturbine::heat_solve::kokkos_1d_heat_conduction(
            axis_size, U_im1, dt, k, deltaU);

        U[0] = U_im1[0];
        U[axis_size - 1] = U_im1[axis_size - 1];

        double residual = openturbine::heat_solve::kokkos_calculate_residual(
            axis_size, U, U_im1);
...
std::vector<double> linspace(T start_in, T end_in, int num_in)
{
    ...
    if (num < 0)
        throw std::invalid_argument(
            "linspace: num_in must be a positive integer");
...
double* kokkos_laplacian(int axis_size, double* U, double dx)
{
    ...
    auto laplacian =
        static_cast<double*>(std::malloc(axis_size * sizeof(double)));
    Kokkos::parallel_for(
...
double kokkos_calculate_residual(int axis_size, double* U, double* U_im1)
{
    ...
    Kokkos::parallel_reduce(
        "residual", axis_size,
        KOKKOS_LAMBDA(const int& i, double& iresidual) {
