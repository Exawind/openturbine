...
struct InterpolateToQuadraturePoints {
    ...
    void operator()(Kokkos::TeamPolicy<>::member_type member) const {
        ...
        Kokkos::parallel_for(
            ...
            InterpolateQPState_u{first_qp, first_node, num_nodes, shape_interp, node_u, qp_u}
        );
        Kokkos::parallel_for(
            Kokkos::TeamThreadRange(member, idx.num_qps),
            InterpolateQPState_uprime{
                first_qp, first_node, num_nodes, shape_deriv, qp_jacobian, node_u, qp_uprime}
        );
        Kokkos::parallel_for(
            Kokkos::TeamThreadRange(member, idx.num_qps),
            InterpolateQPState_r{first_qp, first_node, num_nodes, shape_interp, node_u, qp_r}
        );
        Kokkos::parallel_for(
            Kokkos::TeamThreadRange(member, idx.num_qps),
            InterpolateQPState_rprime{
                first_qp, first_node, num_nodes, shape_deriv, qp_jacobian, node_u, qp_rprime}
        );
