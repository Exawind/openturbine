...
struct CalculateStrain {
    ...
    void operator()(const int i_qp) const {
        auto x0_prime_data = Kokkos::Array<double, 3>{qp_x0_prime_(i_qp, 0), qp_x0_prime_(i_qp, 1), qp_x0_prime_(i_qp, 2)};
        auto x0_prime = Kokkos::View<double[3], Kokkos::MemoryTraits<Kokkos::Unmanaged>>(x0_prime_data.data());
        ...
        auto R_data = Kokkos::Array<double, 4>{qp_r_(i_qp, 0), qp_r_(i_qp, 1), qp_r_(i_qp, 2), qp_r_(i_qp, 3)};
        auto R = Kokkos::View<double[4], Kokkos::MemoryTraits<Kokkos::Unmanaged>>(R_data.data());
        
        auto R_x0_prime_data = Kokkos::Array<double, 3>{};
        auto R_x0_prime = Kokkos::View<double[3], Kokkos::MemoryTraits<Kokkos::Unmanaged>>(R_x0_prime_data.data());
        ...
        auto R_prime_data = Kokkos::Array<double, 4>{qp_r_prime_(i_qp, 0), qp_r_prime_(i_qp, 1), qp_r_prime_(i_qp, 2), qp_r_prime_(i_qp, 3)};
        auto R_prime = Kokkos::View<double[4], Kokkos::MemoryTraits<Kokkos::Unmanaged>>(R_prime_data.data());
        auto e2_data = Kokkos::Array<double, 3>{};
        auto e2 = Kokkos::View<double[4], Kokkos::MemoryTraits<Kokkos::Unmanaged>>{e2_data.data()};
        KokkosBlas::SerialGemv<KokkosBlas::Trans::NoTranspose, KokkosBlas::Algo::Gemv::Default>::invoke(2., E, R_prime, 0., e2);
