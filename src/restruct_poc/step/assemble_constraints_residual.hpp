...
inline void AssembleConstraintsResidual(Solver& solver, Constraints& constraints) {
    ...
    Kokkos::parallel_for("CopyConstraintsResidualToVector", constraints.num, CopyConstraintsResidualToVector{constraints.row_range, Kokkos::subview(solver.R, Kokkos::make_pair(solver.num_system_dofs, solver.num_dofs)), constraints.residual_terms});
}
