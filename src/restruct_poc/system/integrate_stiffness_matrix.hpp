...
struct IntegrateStiffnessMatrix {
    ...
    void operator()(Kokkos::TeamPolicy<>::member_type member) const {
        ...
        Kokkos::parallel_for(Kokkos::TeamThreadMDRange(member, idx.num_nodes, idx.num_nodes), [&](int i_index, int j_index) {
            ...
            auto local_M = Kokkos::View<double[6][6], Kokkos::MemoryTraits<Kokkos::Unmanaged>>(local_M_data.data());
            ...
            for (int k = 0; k < idx.num_qps; ++k) {
                ...
                for(int m = 0;  m < 6; ++m) {
                    for(int n = 0; n < 6; ++n) {
                        local_M(m, n) += K * qp_Kuu_(k_qp, m, n) + P * qp_Puu_(k_qp, m, n) + Q * qp_Quu_(k_qp, m, n) + C * qp_Cuu_(k_qp, m, n) + O * qp_Ouu_(k_qp, m, n);
                    }
                }
            }
            for(int m = 0; m < 6; ++m) {
