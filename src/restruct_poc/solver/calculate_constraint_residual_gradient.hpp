        ...
        switch (cd.type) {
            ...
            case ConstraintType::PrescribedBC:
                u1_data = Kokkos::Array<double, 3>{
                    control(i_constraint, 0), control(i_constraint, 1), control(i_constraint, 2)};
                r1_data = Kokkos::Array<double, 4>{
                    control(i_constraint, 3), control(i_constraint, 4), control(i_constraint, 5),
                    control(i_constraint, 6)};
                break;
            default:
                u1_data = Kokkos::Array<double, 3>{
