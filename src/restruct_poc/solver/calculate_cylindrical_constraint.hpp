...
struct CalculateCylindricalConstraint {
    ...
    void operator()(const int i_constraint) const {
        ...
        const auto x0_data = Kokkos::Array<double, 3>{axis(i_constraint, 0, 0), axis(i_constraint, 0, 1), axis(i_constraint, 0, 2)};
        const auto x0 = View_3::const_type{x0_data.data()};
        const auto y0_data = Kokkos::Array<double, 3>{axis(i_constraint, 1, 0), axis(i_constraint, 1, 1), axis(i_constraint, 1, 2)};
        const auto y0 = View_3::const_type{y0_data.data()};
        const auto z0_data = Kokkos::Array<double, 3>{axis(i_constraint, 2, 0), axis(i_constraint, 2, 1), axis(i_constraint, 2, 2)};
        ...
        //---------------------------------
        {
            ...
            const auto B =
                Kokkos::subview(gradient_terms, i_constraint, Kokkos::ALL, Kokkos::make_pair(node_col_range(i_constraint, 0, 0), node_col_range(i_constraint, 0, 1)));
