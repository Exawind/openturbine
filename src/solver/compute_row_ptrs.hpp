...
[[nodiscard]] inline RowPtrType ComputeRowPtrs(
    ...
    Kokkos::parallel_for(
        "ComputeSystemRowEntries", Kokkos::RangePolicy<typename RowPtrType::execution_space>(0, num_nodes),
    ...
    Kokkos::parallel_for(
        "ComputeConstraintsRowEntries", Kokkos::RangePolicy<typename RowPtrType::execution_space>(0, num_constraints),
    ...
    Kokkos::parallel_scan(
        "ScanRowEntries", Kokkos::RangePolicy<typename RowPtrType::execution_space>(0, num_dofs), ScanRowEntries<RowPtrType>{row_entries, row_ptrs}
    );
