...
struct ContributeForcesToVector {
    ...
    void operator()(size_t i_node) const {
        ...
        if (this_node_freedom_signature == FreedomSignature::NoComponents) {
            ...
            vector(dof_index + 0, 0) -= node_loads(i_node, 0);
            vector(dof_index + 1, 0) -= node_loads(i_node, 1);
            vector(dof_index + 2, 0) -= node_loads(i_node, 2);
            vector(dof_index + 3, 0) -= node_loads(i_node, 3);
            vector(dof_index + 4, 0) -= node_loads(i_node, 4);
            vector(dof_index + 5, 0) -= node_loads(i_node, 5);
            ...
            vector(dof_index + 0, 0) -= node_loads(i_node, 3);
            vector(dof_index + 1, 0) -= node_loads(i_node, 4);
            vector(dof_index + 2, 0) -= node_loads(i_node, 5);
        }
    }
};
