...
TEST(AerodynamicsInterfaceTest, IEA15_Turbine) {
    ...
    for (auto blade : std::views::iota(0U, n_blades)) {
        ...
        const auto& stiffness_matrix = wio_blade["structure"]["elastic_properties"]["stiffness_matrix"];
        ...
        for (auto section : std::views::iota(0U, n_sections)) {
            ...
            blade_builder.AddSection(
                ...
                {{
                    ...
                    {0., 0., mass[section], mass[section] * cm_y[section], -mass[section] * cm_x[section], 0.},
                    {0., 0., mass[section] * cm_y[section], i_edge[section], -i_cp[section], 0.},
                    {0., 0., -mass[section] * cm_x[section], -i_cp[section], i_flap[section], 0.},
                    {-mass[section] * cm_y[section], mass[section] * cm_x[section], 0., 0., 0., i_plr[section]}
                }},
                {{
                    {K11[section], K12[section], K13[section], K14[section], K15[section], K16[section]},
                    {K12[section], K22[section], K23[section], K24[section], K25[section], K26[section]},
                    {K13[section], K23[section], K33[section], K34[section], K35[section], K36[section]},
                    {K14[section], K24[section], K34[section], K44[section], K45[section], K46[section]},
                    {K15[section], K25[section], K35[section], K45[section], K55[section], K56[section]},
                    {K16[section], K26[section], K36[section], K46[section], K56[section], K66[section]},
                }},
                interfaces::components::ReferenceAxisOrientation::Z
            );
        }
    ...
    const auto tower_wall_thickness = wio_tower["structure"]["layers"][0]["thickness"]["values"].as<std::vector<double>>();
    const auto tower_material_name = wio_tower["structure"]["layers"][0]["material"].as<std::string>();
    auto& ref_axis = wio_tower["reference_axis"];
    ...
    const auto z_values = ref_axis["z"]["values"].as<std::vector<double>>();
    for (auto i : std::views::iota(0U, axis_grid.size())) {
        tower_builder.AddRefAxisPoint(axis_grid[i], {x_values[i], y_values[i], z_values[i]}, interfaces::components::ReferenceAxisOrientation::Z);
    }
    ...
    auto tower_material = std::find_if(std::begin(wio["materials"]), std::end(wio["materials"]), [&tower_material_name](const YAML::Node& node) { return node["name"].as<std::string>() == tower_material_name; })->as<YAML::Node>();
    ...
    for (auto i : std::views::iota(0U, tower_diameter_grid.size())) {
        const auto section = beams::GenerateHollowCircleSection(tower_diameter_grid[i], elastic_modulus, shear_modulus, density, tower_diameter_values[i], tower_wall_thickness[i], poisson_ratio);

        tower_builder.AddSection(tower_diameter_grid[i], section.M_star, section.C_star, interfaces::components::ReferenceAxisOrientation::Z);
    }
    
    auto& wio_hub = wio["components"]["hub"];
    turbine_builder.SetConeAngle(wio_hub["cone_angle"].as<double>());
    turbine_builder.SetHubDiameter(wio_hub["diameter"].as<double>());
    turbine_builder.SetRotorApexToHub(0.);
    ...
    turbine_builder.SetTowerTopToRotorApex(wio_drivetrain["outer_shape"]["distance_tt_hub"].as<double>());
    turbine_builder.SetShaftTiltAngle(wio_drivetrain["outer_shape"]["uptilt"].as<double>());
    ...
    for (auto& af : airfoil_io) {
        ...
        std::ranges::transform(aoa, std::begin(aoa), [](auto degrees) { return degrees * std::numbers::pi / 180.; });
        ...
        aero_sections.emplace_back(id, s, chord, twist, section_offset_x, section_offset_y, aerodynamic_center, aoa, cl, cd, cm);
        ++id;
    }

    auto aero_inputs = std::vector<interfaces::components::AerodynamicBodyInput>{};
    for (auto blade : std::views::iota(0U, n_blades)) {
        auto beam_node_ids = std::vector<size_t>{};
        std::ranges::transform(turbine_interface.Turbine().blades[blade].nodes, std::back_inserter(beam_node_ids), [](auto& node_data) { return node_data.id; }); 
        aero_inputs.emplace_back(blade, beam_node_ids, aero_sections);
    }
    auto aero_interface = interfaces::components::Aerodynamics(aero_inputs, turbine_interface.GetModel().GetNodes());
    ...
    for (auto i : std::views::iota(1U, n_steps)) {
        ...
        aero_interface.SetInflowFromFunction([t, &inflow](const std::array<double, 3> pos) { return inflow.Velocity(t, pos); });

        aero_interface.CalculateAerodynamicLoads(fluid_density);

        aero_interface.CalculateNodalLoads();

        aero_interface.AddNodalLoadsToState(turbine_interface.GetHostState());
