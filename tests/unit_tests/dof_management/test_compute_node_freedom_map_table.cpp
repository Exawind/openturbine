...
TEST(TestComputeNodeFreedomMapTable, OneNode) {
    ...
    const auto host_node_freedom_map_table = Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), state.node_freedom_map_table);
...
TEST(TestComputeNodeFreedomMapTable, FourNodes) {
    ...
    const auto mirror_node_freedom_allocation_table =
        Kokkos::create_mirror_view(state.node_freedom_allocation_table);
