...
TEST(CalculateForceFCTests, OneNode) {
    ...
    const auto Cuu_host = Kokkos::View<const double[1][6][6], Kokkos::HostSpace>(Cuu_data.data());
    const auto Cuu_mirror = Kokkos::create_mirror(Cuu);
    Kokkos::deep_copy(Cuu_mirror, Cuu_host);
    Kokkos::deep_copy(Cuu, Cuu_mirror);

    const auto strain = Kokkos::View<double[1][6]>("strain");
    constexpr auto strain_data = std::array{37., 38., 39., 40., 41., 42.};
    const auto strain_host = Kokkos::View<const double[1][6], Kokkos::HostSpace>(strain_data.data());
    ...
    const auto M_tilde = Kokkos::View<double[1][3][3]>("M_tilde");
    const auto N_tilde = Kokkos::View<double[1][3][3]>("N_tilde");
    ...
    const auto FC_exact = Kokkos::View<const double[1][6], Kokkos::HostSpace>(FC_exact_data.data());
    ...
    const auto M_tilde_exact =
        Kokkos::View<const double[1][3][3], Kokkos::HostSpace>(M_tilde_exact_data.data());
    ...
    const auto N_tilde_exact =
        Kokkos::View<const double[1][3][3], Kokkos::HostSpace>(N_tilde_exact_data.data());
