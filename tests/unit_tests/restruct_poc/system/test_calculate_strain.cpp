...
TEST(CalculateStrainTests, OneNode) {
    ...
    const auto r_host = Kokkos::View<const double[1][4], Kokkos::HostSpace>(r_data.data());
    const auto r_mirror = Kokkos::create_mirror(r);
    Kokkos::deep_copy(r_mirror, r_host);
    Kokkos::deep_copy(r, r_mirror);

    const auto r_prime = Kokkos::View<double[1][4]>("r_prime");
    constexpr auto r_prime_data = std::array{11., 12., 13., 14.};
    const auto r_prime_host =
        Kokkos::View<const double[1][4], Kokkos::HostSpace>(r_prime_data.data());
    ...
    const auto strain_exact =
        Kokkos::View<const double[1][6], Kokkos::HostSpace>(strain_exact_data.data());
