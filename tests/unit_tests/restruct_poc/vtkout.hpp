...
inline void BeamsWriteVTK(Beams& beams, std::string filename) {
    ...
    for (size_t i = 0; i < beams.num_elems; ++i) {
        ...
        pts.push_back(static_cast<vtkIdType>(idx.qp_range.first));       // first qp
        pts.push_back(static_cast<vtkIdType>(idx.qp_range.second - 1));  // last qp
        for (size_t j = idx.qp_range.first; j < idx.qp_range.second - 2; ++j) {
            pts.push_back(static_cast<vtkIdType>(j));
        }
        gd->InsertNextCell(VTK_LAGRANGE_CURVE, static_cast<vtkIdType>(idx.num_qps), pts.data());
    }
