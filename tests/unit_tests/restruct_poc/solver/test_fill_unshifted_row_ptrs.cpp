...
TEST(FillUnshiftedRowPtrs, Shift1By1) {
    ...
    const auto old_row_ptrs_host = Kokkos::View<const size_t[num_initial_rows + 1U], Kokkos::HostSpace>(old_row_ptrs_host_data.data());
    ...
    Kokkos::parallel_for("FillUnshiftedRowPtrs", num_final_rows + 1U, FillUnshiftedRowPtrs<Kokkos::View<size_t*>>{num_initial_rows, old_row_ptrs, new_row_ptrs});

    constexpr auto new_row_ptrs_exact_host = std::array<size_t, num_final_rows + 1U>{0U, 5U, 5U};
    const auto new_row_ptrs_exact = Kokkos::View<const size_t[num_final_rows + 1U], Kokkos::HostSpace>(new_row_ptrs_exact_host.data());
