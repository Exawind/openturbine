...
#include "src/restruct_poc/solver/populate_sparse_row_ptrs_col_inds_transpose.hpp"
...
TEST(PopulateSparseRowPtrsColInds_Transpose, DenseSquare) {
    ...
    const auto row_ptrs_host = Kokkos::View<const size_t[rows + 1U], Kokkos::HostSpace>(row_ptrs_host_data.data());
    Kokkos::deep_copy(row_ptrs, row_ptrs_host);

    constexpr auto col_inds_host_data = std::array<size_t, nnz>{0U, 1U, 2U, 3U, 4U, 0U, 1U, 2U, 3U, 4U, 0U, 1U, 2U, 3U, 4U, 0U, 1U, 2U, 3U, 4U, 0U, 1U, 2U, 3U, 4U};
    const auto col_inds = Kokkos::View<size_t[nnz]>("col_inds");
    const auto col_inds_host = Kokkos::View<const size_t[nnz], Kokkos::HostSpace>(col_inds_host_data.data());
    ...
    Kokkos::parallel_for("PopulateSparseRowPtrsColInds_Transpose", 1, PopulateSparseRowPtrsColInds_Transpose<Kokkos::View<size_t*>, Kokkos::View<size_t*>>{rows, cols, row_ptrs, col_inds, col_count, temp_row_ptr, row_ptrs_trans, col_inds_trans});


    constexpr auto row_ptrs_trans_exact_data = row_ptrs_host_data;
    const auto row_ptrs_trans_exact = Kokkos::View<const size_t[cols + 1U], Kokkos::HostSpace>(row_ptrs_trans_exact_data.data());
    ...
    const auto col_inds_trans_exact = Kokkos::View<const size_t[nnz], Kokkos::HostSpace>(col_inds_trans_exact_data.data());
