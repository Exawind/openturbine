...
#include "src/system/calculate_external_forces.hpp"

namespace openturbine::tests {

TEST(CalculateExternalForcesTests, OneNode) {
    ...
    const auto FE = Kokkos::View<double[1][1][6]>("FE");

    Kokkos::parallel_for(
        "CalculateExternalForces", 1,
        CalculateExternalForces{
            0, Muu, u_ddot, omega, omega_dot, eta_tilde, omega_tilde, omega_dot_tilde, rho, eta, FE
        }
    );
    ...
    constexpr auto FE_exact_data = std::array{-2984., 32., 2922., 20624., -2248., 22100.};
    const auto FE_exact =
        Kokkos::View<const double[1][1][6], Kokkos::HostSpace>(FE_exact_data.data());
