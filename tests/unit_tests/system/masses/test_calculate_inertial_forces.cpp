...
TEST(CalculateInertialForcesTestsMasses, OneNode) {
    ...
    const auto eta_tilde = CreateView<double[3][3]>("eta_tilde", std::array{46., 47., 48., 49., 50., 51., 52., 53., 54.});
    const auto rho = CreateView<double[3][3]>("rho", std::array{55., 56., 57., 58., 59., 60., 61., 62., 63.});
    const auto eta = CreateView<double[3]>("eta", std::array{64., 65., 66.});
    const auto omega_tilde = CreateView<double[3][3]>("omega_tilde", std::array{0., -42., 41., 42., 0., -40., -41., 40., 0.});
    const auto omega_dot_tilde = CreateView<double[3][3]>("omega_dot_tilde", std::array{0., -45., 44., 45., 0., -43., -44., 43., 0.});
    ...
    const auto FI_exact = Kokkos::View<double[6], Kokkos::HostSpace>::const_type(FI_exact_data.data());
