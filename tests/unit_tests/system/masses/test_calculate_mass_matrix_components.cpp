...
struct ExecuteCalculateMassMatrixComponents {
    ...
    Kokkos::View<double[1][3]> eta;
    Kokkos::View<double[1][3][3]> rho;
    Kokkos::View<double[1][3][3]> eta_tilde;

    KOKKOS_FUNCTION
    void operator()(size_t) const {
        masses::CalculateMassMatrixComponents{i_elem, Muu, eta, rho, eta_tilde}();
    }
};
...
TEST(CalculateMassMatrixComponentsMassesTests, OneQuadPoint) {
    ...
    Kokkos::parallel_for(
        ...
        ExecuteCalculateMassMatrixComponents{0, Muu, eta, rho, eta_tilde}
    );
