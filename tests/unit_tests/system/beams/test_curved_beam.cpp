...
TEST(CurvedBeamTests, CalculateJacobianForCurvedBeam) {
    const auto node_position_rotation = CreateView<double[kNumElems][kNumNodes][7]>("node_position_rotation", kCurvedBeamNodes_data);
    const auto shape_derivative = CreateView<double[kNumElems][kNumNodes][kNumQPs]>("shape_derivative", kDerivWeightsFlat);
    const auto kNumNodes_per_elem = CreateView<size_t[kNumElems]>("number_of_nodes_per_element", std::array{kNumNodes});
    const auto kNumQPs_per_elem = CreateView<size_t[kNumElems]>("number_of_quadrature_points_per_element", std::array{kNumQPs});

    const auto qp_position_derivative = Kokkos::View<double[kNumElems][kNumQPs][3]>("position_derivative");
    ...
    const auto qp_jacobian_mirror = Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), qp_jacobian);
    ...
    const auto qp_position_derivative_mirror = Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), qp_position_derivative);
    for (size_t qp = 0; qp < kNumQPs; ++qp) {
...
TEST(CurvedBeamTests, IntegrateResidualVectorForCurvedBeam) {
    ...
    const auto shape_interp = CreateView<double[kNumNodes][kNumQPs]>("shape_interp", kInterpWeightsFlat);
    const auto shape_deriv = CreateView<double[kNumNodes][kNumQPs]>("shape_deriv", kDerivWeightsFlat);
...
TEST(CurvedBeamTests, IntegrateStiffnessMatrixForCurvedBeam) {
    ...
    const auto stiffness_matrix_terms_mirror = Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), stiffness_matrix_terms);
