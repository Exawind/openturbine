...
TEST(HeatSolve_linspace, argtypes)
{
    ...
    // for start, stop and step. Successfully compiling and linking means the
    // test passes.
    // std::vector<double> test_lin1 = openturbine::heat_solve::linspace(1, 2,
    // 3);
    std::vector<double> test_lin2 =
        openturbine::heat_solve::linspace(1.0, 2.0, 3);
    // std::vector<double> test_lin3 = openturbine::heat_solve::linspace(1, 2.0,
    // 3);

    // this should handle negative arguments just the same
    std::vector<double> test_lin4 =
...
TEST(HeatSolve_linspace, values)
{
    ...
    // linspace should equally distribute points including at the beginning and
    // end.
    // Test the trivial case of 3 points: 0.0, 1.0, 2.0
    truth_vec = std::vector{0.0, 1.0, 2.0};
    ASSERT_EQ(openturbine::heat_solve::linspace(0.0, 2.0, 3), truth_vec);

    // It should go in the opposite direction if needed
    truth_vec = std::vector{2.0, 1.0, 0.0};
    ASSERT_EQ(openturbine::heat_solve::linspace(2.0, 0.0, 3), truth_vec);

    // 0 step size should yield a 0-size vector
    std::vector<double> zero_lin =
        openturbine::heat_solve::linspace(0.0, 2.0, 0);
    ASSERT_EQ(zero_lin.size(), 0);

    // 1 step size should yield a 1-size vector with the starting point
    std::vector<double> one_lin =
        openturbine::heat_solve::linspace(0.0, 2.0, 1);
...
TEST(HeatSolve, FullSolve)
{
    ...
    ASSERT_NEAR(
        std::accumulate(U, U + axis_size, 0.0),
        test_temp * axis_size / side_length, 1e-6);

    // This heat model is actually just a line. For an odd number of grid
    // points, the middle point should have a value equal to half the difference
    // of the end points.
    test_temp = 10.0;
    U = static_cast<double*>(std::malloc(axis_size * sizeof(double)));
    U = openturbine::heat_solve::heat_conduction_solver(
