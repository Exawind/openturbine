...
TEST(CalculateDisplacement, OneNode) {
    ...
    const auto q_delta_host = Kokkos::View<double[1][6], Kokkos::HostSpace>::const_type(q_delta_host_data.data());
    ...
    const auto q_prev_host = Kokkos::View<double[1][7], Kokkos::HostSpace>::const_type(q_prev_host_data.data());
    ...
    constexpr auto q_exact_data = std::array{9., 12., 15., -20.510952969318009, -6.4827969278626361, -5.141528734877415,  -6.696180594260384};
    const auto q_exact = Kokkos::View<double[1][7], Kokkos::HostSpace>::const_type(q_exact_data.data());
