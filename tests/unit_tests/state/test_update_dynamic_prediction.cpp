...
TEST(UpdateDynamicPrediction, OneNode) {
    ...
    const auto node_freedom_allocation_table = CreateView<FreedomSignature[1]>("nfat", std::array{FreedomSignature::AllComponents});
    const auto node_freedom_map_table = CreateView<size_t[1]>("nfmt", std::array{0UL});
    const auto x_delta = CreateView<double[6][1]>("x_delta", std::array{1., 2., 3., 4., 5., 6.});
