...
TEST(ComputeColInds, OneElementOneNode) {
    ...
    const auto node_freedom_map_table = CreateView<size_t[1]>("node_freedom_map_table", std::array{0UL});
    const auto num_nodes_per_element = CreateView<size_t[1]>("num_nodes_per_element", std::array{1UL});
...
TEST(ComputeColInds, OneElementTwoNodes) {
    ...
    const auto node_freedom_map_table = CreateView<size_t[2]>("node_freedom_map_table", std::array{0UL, 6UL});
    const auto num_nodes_per_element = CreateView<size_t[1]>("num_nodes_per_element", std::array{2UL});
    const auto node_state_indices = CreateView<size_t[1][2]>("node_state_indices", std::array{0UL, 1UL});
    const auto row_ptrs = CreateView<int[13]>("row_ptrs", std::array{0, 12, 24, 36, 48, 60, 72, 84, 96, 108, 120, 132, 144});
...
TEST(ComputeColInds, TwoElementsTwoNodesNoOverlap) {
    ...
    const auto node_freedom_map_table = CreateView<size_t[4]>("node_freedom_map_table", std::array{0UL, 6UL, 12UL, 18UL});
    const auto num_nodes_per_element = CreateView<size_t[2]>("num_nodes_per_element", std::array{2UL, 2UL});
    const auto node_state_indices = CreateView<size_t[2][2]>("node_state_indices", std::array{0UL, 1UL, 2UL, 3UL});
    const auto row_ptrs = CreateView<int[25]>("row_ptrs", std::array{0,   12,  24,  36,  48,  60,  72,  84,  96,  108, 120, 132, 144, 156, 168, 180, 192, 204, 216, 228, 240, 252, 264, 276, 288});
...
TEST(ComputeColInds, TwoElementsTwoNodesOverlap) {
    ...
    const auto node_freedom_map_table = CreateView<size_t[3]>("node_freedom_map_table", std::array{0UL, 6UL, 12UL});
    const auto num_nodes_per_element = CreateView<size_t[2]>("num_nodes_per_element", std::array{2UL, 2UL});
    const auto node_state_indices = CreateView<size_t[2][2]>("node_state_indices", std::array{0UL, 1UL, 1UL, 2UL});
    const auto row_ptrs = CreateView<int[19]>("row_ptrs", std::array{0,   12,  24,  36,  48,  60,  72,  90,  108, 126, 144, 162, 180, 192, 204, 216, 228, 240, 252});
...
TEST(ComputeColInds, OneElementOneNode_OneConstraint) {
    ...
    const auto base_node_freedom_table = CreateView<size_t[1][6]>("base_node_freedom_table", std::array{0UL, 1UL, 2UL, 3UL, 4UL, 5UL});
    const auto target_node_freedom_table = CreateView<size_t[1][6]>("target_node_freedom_table", std::array{6UL, 7UL, 8UL, 9UL, 10UL, 11UL});
    const auto row_range = CreateView<Kokkos::pair<size_t, size_t>[1]>("row_range", std::array{Kokkos::make_pair(0UL, 6UL)});
    const auto row_ptrs = CreateView<int[19]>("row_ptrs", std::array{0,   12,  24,  36,  48,  60,  72,  84,  96, 108, 120, 132, 144, 156, 168, 180, 192, 204, 216});
...
TEST(ComputeColInds, OneElementOneNode_TwoConstraint) {
    ...
    const auto target_active_dofs = CreateView<size_t[2]>("target_active_dofs", std::array{6UL, 6UL});
    const auto base_node_freedom_table = CreateView<size_t[2][6]>("base_node_freedom_table", std::array{0UL, 1UL, 2UL, 3UL, 4UL, 5UL, 0UL, 1UL, 2UL, 3UL, 4UL, 5UL});
    const auto target_node_freedom_table = CreateView<size_t[2][6]>("target_node_freedom_table", std::array{6UL, 7UL, 8UL, 9UL, 10UL, 11UL, 12UL, 13UL, 14UL, 15UL, 16UL, 17UL});
    const auto row_range = CreateView<Kokkos::pair<size_t, size_t>[2]>("row_range", std::array{Kokkos::make_pair(0UL, 6UL), Kokkos::make_pair(6UL, 12UL)});
    const auto row_ptrs = CreateView<int[31]>("row_ptrs", std::array{0,   18,  36,  54,  72,  90,  108, 120, 132, 144, 156, 168, 180, 192, 204, 216, 228, 240, 252, 264, 276, 288, 300, 312, 324, 336, 348, 360, 372, 384, 396});
